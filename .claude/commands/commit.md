---
allowed-tools: Bash(git status:*), Bash(git diff:*), Bash(git log:*), Bash(git branch:*), Bash(git fetch:*), Bash(git rebase:*), Bash(git add:*), Bash(git commit:*)
description: origin rebase 체크 → 스테이징 → conventional commit 자동화
---
...
`git commit -m "<메시지>"` 실행.
완료 후 `git log --oneline -3` 을 출력해 결과를 확인한다.
push는 하지 않는다.
